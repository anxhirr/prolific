...
interface EnhancedFractalTrendPanelProps {
  chartData: CandlestickData[];
}

export function EnhancedFractalTrendPanel({ chartData }: EnhancedFractalTrendPanelProps) {
  ...
  const handleEnhancedFractalTrendAnalyze = () => {
    if (chartData.length < 5) {
      toast({
        ...
        description: `Need at least 5 candles for enhanced fractal trend analysis. Current: ${chartData.length}`,
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Trend: ${analysisResult.trend} with ${(analysisResult.confidence * 100).toFixed(1)}% confidence (${analysisResult.detailedAnalysis?.highSequence.length || 0} highs, ${analysisResult.detailedAnalysis?.lowSequence.length || 0} lows)`,
      });
    } catch (error) {
    ...
    <aside className="w-[340px] border-r border-border bg-card flex flex-col">
      <div className="flex-shrink-0 p-4 border-b">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold">Enhanced Fractal Trend</h3>
          </div>
          <Button
            variant="ghost"
            size="sm"
      ...
      {showSettings && (
        <div className="flex-shrink-0 p-4 border-b bg-muted/30">
          <h4 className="text-sm font-medium mb-3">Enhanced Analysis Settings</h4>
          <div className="space-y-3">
            <div className="text-xs text-muted-foreground">
              <p>• Analyzes ALL fractals in sequence, not just recent ones</p>
